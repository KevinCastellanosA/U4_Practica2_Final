...
public class ArbolGeneral {
   private NodoGeneral raiz,padre;
      ...
      if(vector.length == 2){
       for (int i = 1; i < 2; i++) {
        padre = padre.Hijos(vector[i].charAt(0));
         ...
         return BuscarRecursivo(path.substring(2), padre);
     }
     return null;
    }
    
    private NodoGeneral BuscarRecursivo(String path, NodoGeneral Auxiliar){
        if(Auxiliar == null) return null;
        if (path.length() == 1)return Auxiliar.Hijos(path.charAt(0));
        return BuscarRecursivo( path.substring(2),Auxiliar.Hijos(path.charAt(0)));
    }
    ...
    public NodoGeneral BuscarPath(String path) {
        ...
        return BuscarRecursivo( path.substring(4),raiz.Hijos(path.charAt(2)));
    }
